...
# to build on mac 
# docker build -t zmk --build-arg ZEPHYR_VERSION=3.5.0 --build-arg ZEPHYR_SDK_VERSION=0.16.1 --build-arg ARCHITECTURE=arm .
# docker run -it --rm -v /Users/walter/projects/zmk-config:/workspace zmk bash -c "cd /workspace && west init -l config && west update && west zephyr-export && west build -s zmk/app -b nice_nano_v2 -- -DSHIELD=cp44 -DZMK_CONFIG=/workspace/config"
